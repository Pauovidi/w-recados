...
const views = [
  ...
  { path: '/repartidor/acceso', label: 'Repartidor', icon: Truck },
];
    ...
    <div className="fixed bottom-3 left-1/2 z-[80] flex w-[calc(100%-1rem)] max-w-lg -translate-x-1/2 items-center gap-1 rounded-2xl border border-white/10 bg-slate-950/95 p-1.5 text-white shadow-2xl backdrop-blur md:w-auto">
      ...
      {views.map((view) => {
        const active = view.path === '/'
          ? location.pathname === '/' || location.pathname.startsWith('/pedido')
          : view.path === '/repartidor/acceso'
            ? location.pathname.startsWith('/repartidor')
